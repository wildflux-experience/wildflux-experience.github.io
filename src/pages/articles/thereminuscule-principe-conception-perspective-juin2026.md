...
![image_illu_thereminuscule](/images/thereminuscule_principe_conception_perspective_juin2026/illustration_principe_thereminscule_annote_200dpi.png)
...
![schema_fonction_thereminscule](/images/thereminuscule_principe_conception_perspective_juin2026/schema_fonction_thereminscule.png)
...
Avant son traitement par les *Processeurs*, la "note brute" issue des sources, est d’abord exprimée sous forme de valeur numérique "8-bits" pouvant aller de 0 à 255. Ici, 0 représente la plus petite valeur numérique que l’on s’attend à lire dans le flux de donnée entrant, 255 représente la plus grande attendue.
...
L’exemple qui suit illustre la conversion d’une valeur numérique "8-bits" (de 0 à 255) en une note de musique par le *Processeur* nommé `Scaler`.  La valeur prise pour exemple est 168, qui se situe environ au 2/3 de la plage des valeurs possibles en sortie des sources. Dans cet exemple, le Thereminuscule est réglé pour jouer sur une gamme de *Do* majeur à partir du 2ème octave (ou *C2* en anglais et 36 en MIDI). La note finale élue en fin de conversion correspond à la valeur 168, soit la note de *Si* du 4ème octave (ou *B4* en anglais et 71 en MIDI)

▶ Paramètres de réglage Thereminuscule utilisés par le `Scaler`:

| Paramètre | Variable dans le programme | Valeur exemple |
| --- | --- | --- |
| La fondamentale | `fonda` | Do → C2 = 36 en MIDI |
...
```python
  ...
  # ETAPE 1 : de la valeur brute à l'identificant de la note
  ...
  # ETAPE 2 : de l'identifiant à la note finale 
  #           en fonction de la fondamentale et de la gamme
...
Une fois les traitements effectués par les *Processeurs*, la note finale et les paramètres de modulation ne sont encore que des nombres dans un programme informatique. Il s'agit alors de transcrire ces suites de nombres en signaux réellement exploitables par des logiciels externes de musique ou d'effets visuels : c'est le rôle des *Traducteurs*.

Dans le domaine de la MAO (Musique Assistée par Ordinateur), le standard le plus utilisé est le MIDI (Musical Instrument Digital Interface). Ce protocole naît au début des années 1980, avec la volonté de normaliser les échanges entre les instruments électroniques de musique alors disponibles, comme les synthétiseurs, les contrôleurs, les séquenceurs, les pédales d'effet, etc.

Pour ce faire, le *Thereminuscule* repose sur une librairie en Python, appelée [Mido](https://mido.readthedocs.io/en/stable/), qui gère automatiquement la transcription des signaux internes en signaux MIDI externes. Ceux-ci peuvent alors être redirigés vers l'instrument ou le logiciel de création musicale (ou DAW, pour Digital Audio Workstation) de votre choix.

#### Les Performeurs

Afin de rendre le *Thereminuscule* interactif et jouable en "live", l'ensemble de ses paramètres internes peut être modifié à la volée, et l'effet d'un changement est immédiat.

Pour ce faire, ces paramètres sont exposés dans un fichier au format [JSON](https://www.json.org/json-fr.html), qui est lu en continu. Ce fichier est alors éditable par n'importe quelle application ou dispositif externe afin de permettre au joueur d'influer en direct sur les mélodies produites.

De base, le programme est fourni avec une interface simple et intuitive pour être contrôlé directement via l'ordinateur qui l'exécute (Figure 4). La conception d'un pad de contrôle dédié, ou l'adaptation de systèmes déjà existants (ex. : [OpenDeck](https://github.com/shanteacontrols/OpenDeck)), afin de limiter l'utilisation de la souris et du clavier lorsqu'il est joué en "live", fait partie des prochaines étapes d'amélioration du *Thereminuscule*.

![ui_thereminuscule_part_live](/images/thereminuscule_principe_conception_perspective_juin2026/screenshot_thereminuscule_ui_part_live.png)

![ui_thereminuscule_part_sources](/images/thereminuscule_principe_conception_perspective_juin2026/screenshot_thereminuscule_ui_part_sources.png)

> ***Figure 4**. Vue d’ensemble des paramètres de réglage du Thereminuscule qui peuvent être modifiés en live par les Performeurs. Dans cet exemple il s’agit d’une simple interface pour contrôler le Thereminuscule avec la souris ou un écran tactile depuis un ordinateur / tablette / téléphone. (Haut) Réglage des paramètres principaux en mode "LIVE". (Bas) Réglage avancé des sources utilisés en mode "SOURCES".*
...
Le **Didgeridoo numérique**, enfin, mobilise le souffle comme geste expressif. En soufflant ou en faisant vibrer ses lèvres plus ou moins fort dans un tube, l’utilisateur influence la hauteur, l’intensité ou la vitesse de la ligne mélodique.
...
Avec **Pluie et évaporation musicale**, l’eau devient la matière première. Les gouttes, l’humidité puis le séchage progressif d’une surface produisent un flux lent et changeant.
...
La **Branche musicale** traduit la flexion d’une tige ou d’une structure souple, soit par l'action d'un geste humain ou des mouvement de l'air ambiant en signal musical. L'évocation du vivant est ici aussi au centre de ce Nano-Module.

La **Méduse suspendue** donne forme aux mouvements de l’air grâce à une sculpture légère, sensible aux courants naturel ou ou volontaires par le biais de ventilateurs.

La **Constellation magnétique** met en scène une force invisible : des aimants déplacés sur une carte représentant la voute celeste composent des configurations spécifiques mesurables, qui influencent la mélodie produite.

## Pour conclure

Le *Thereminuscule* et ses *Nano-Modules* constituent une base d'exploration et d'expérimentation artistique, qui propose d'exploiter l'intuition musicale des participants ou l'effet de phénomènes physiques naturels issus du monde réel pour créer des mélodies originales.

L'ensemble des sources des développements technologiques est mis à disposition publiquement sur le GitHub du projet (https://github.com/wildflux-experience), afin de permettre à n'importe qui de s'approprier le système, de le faire évoluer, ou encore d'imaginer de nouvelles façons d'en jouer et de nouvelles formes à lui donner.

Aussi, parce que ce projet se situe à l'intersection de plusieurs disciplines scientifiques, comme l'informatique, les mathématiques, la physique ou, plus indirectement, la biologie et l'écologie, il constitue une plateforme intéressante pour vulgariser ces sujets et amener le public à les questionner et à s'informer de manière ludique.
